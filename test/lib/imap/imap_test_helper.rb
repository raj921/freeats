...
module ImapTestHelper
  ...
  PARSED_SIMPLE_MESSAGE = Imap::Message.new(
    ...
    headers: [
      ...
      { "Received" => "from mail-sor-f69.google.com (mail-sor-f69.google.com. [209.85.220.69])        by mx.google.com with SMTPS id p25-20020a056512329900b005101702db2dsor437450lfe.18.2024.01.24.17.59.36        for <developers@example.com>        (Google Transport Security);        Wed, 24 Jan 2024 17:59:36 -0800 (PST)" },
      ...
      { "Received" => "from mail-sor-f41.google.com (mail-sor-f41.google.com. [209.85.220.41])        by mx.google.com with SMTPS id o16-20020a198c10000000b005101c043967sor55353lfd.12.2024.01.24.17.59.33        for <developers@example.com>        (Google Transport Security);        Wed, 24 Jan 2024 17:59:33 -0800 (PST)" },
      ...
      { "ARC-Authentication-Results" => "i=3; mx.google.com;       dkim=pass header.i=@example.com header.s=google header.b=GmVThoSo;       arc=pass (i=2 spf=pass spfdomain=example.com dkim=pass dkdomain=example.com dmarc=pass fromdomain=example.com);       spf=pass (google.com: domain of developers+bncbdv4xz5qtaibbb4by6wqmgqenbnhvha@example.com designates 209.85.220.69 as permitted sender) smtp.mailfrom=developers+bncBDV4XZ5QTAIBBB4BY6WQMGQENBNHVHA@example.com;       dmarc=pass (p=NONE sp=NONE dis=NONE) header.from=example.com" },
      ...
      { "ARC-Authentication-Results" => "i=2; mx.google.com;       dkim=pass header.i=@example.com header.s=google header.b=TzxdsK4Z;       spf=pass (google.com: domain of admin@mail.com designates 209.85.220.41 as permitted sender) smtp.mailfrom=admin@mail.com;       dmarc=pass (p=NONE sp=NONE dis=NONE) header.from=example.com" },
      { "DKIM-Signature" => "v=1; a=rsa-sha256; c=relaxed/relaxed;        d=example.com; s=google; t=1706147976; x=1706752776; darn=example.com;        h=list-unsubscribe:list-archive:list-help:list-post:list-id         :mailing-list:precedence:x-original-authentication-results         :x-original-sender:to:subject:message-id:date:from:mime-version:from         :to:cc:subject:date:message-id:reply-to;        bh=JrDfgKgD3fxIaEwe/UVuDOVXvt0kUiZ+K3u86egdlIM=;        b=GmVThoSoDdRv7FgVgku7eW+1+zkHN3R2yu3InunfW/0iJnFu1srWGNlIbspQHulGsz         vbMnZFqUsBRAO8avfbOUzBLUw3jjwHn6Rp4Bz6ZKj7Ekjcbh4neqP9WqO4QGJ1iHZgzX         fHmpf7HLZUKbDVyp9qq54cBOD5kwAFuoSjXMvkDnT951QNzc+90zhjRbKOuaVrHnmYqf         6MsHwj6LGMjCzOsOwCZYQwtVQmNN1VnC0k/6PZfN3SYr6q1WuCz5BLOToJN1URnUZTcZ         I50J7dSqnzup+lnL05TlObQwepe2yImtTHL6z2t3cMgdhVM3BW65AhZAOthPvfIeo1oN         X58g==" },
      ...
      { "ARC-Authentication-Results" => "i=1; mx.google.com;       dkim=pass header.i=@example.com header.s=google header.b=TzxdsK4Z;       spf=pass (google.com: domain of admin@mail.com designates 209.85.220.41 as permitted sender) smtp.mailfrom=admin@mail.com;       dmarc=pass (p=NONE sp=NONE dis=NONE) header.from=example.com" },
      ...
      { "X-Original-Authentication-Results" => "mx.google.com;       dkim=pass header.i=@example.com header.s=google header.b=TzxdsK4Z;       spf=pass (google.com: domain of admin@mail.com designates 209.85.220.41 as permitted sender) smtp.mailfrom=admin@mail.com;       dmarc=pass (p=NONE sp=NONE dis=NONE) header.from=example.com" },
      { "Precedence" => "list" },
      ...
      { "List-Help" => "<https://support.google.com/a/example.com/bin/topic.py?topic=25838>, <mailto:developers+help@example.com>" },
      { "List-Archive" => "<https://groups.google.com/a/example.com/group/developers/>" },
      { "List-Unsubscribe" => "<mailto:googlegroups-manage+494641124923+unsubscribe@googlegroups.com>, <https://groups.google.com/a/example.com/group/developers/subscribe>" }
    ],
    autoreply_headers: {
      auto_submitted: "",
      x_autoreply: "",
